...
describe("constructor", () => {
    test("Constructs dice", () => {
        const dice = new Dice({ remaining: [6, 3], isDoubles: false });
        expect(dice).toMatchObject({ remaining: [6, 3], isDoubles: false });
    });
...
describe("getLargestRemaining", () => {
    test("Gets largest number", () => {
        const dice = new Dice({ remaining: [1, 2], isDoubles: false });
        const dice2 = new Dice({ remaining: [2, 1], isDoubles: false });
...
describe("use", () => {
    ...
    test("Removes the number from remaining for doubles", () => {
        const dice = new Dice({ remaining: [1, 1, 1, 1], isDoubles: true });
        dice.use(1);
